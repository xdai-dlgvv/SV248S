...
class Polygon(Shape):
    ...
    def resize_center(self, center_x, center_y, r, width_bound, height_bound):
        self._points[:, 0] = center_x + r * (self._points[:, 0] - center_x)
        self._points[:, 1] = center_y + r * (self._points[:, 1] - center_y)
        self._points[:, 0] = np.clip(self._points[:, 0], 0, width_bound)
        self._points[:, 1] = np.clip(self._points[:, 1], 0, height_bound)
