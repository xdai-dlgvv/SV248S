...
def calculate_location_err(reg1: Shape, reg2: Shape, bounds: Optional[Tuple[int, int]] = None):
    ...
    if isinstance(reg1, Rectangle):
        data1 = np.round(reg1._data)
        cx_1, cy_1 = data1[0, 0] + data1[2, 0] / 2, data1[1, 0] + data1[3, 0] / 2

    elif isinstance(reg1, Polygon):
        data1 = np.round(reg1._points)
        x1, y1, x2, y2 = np.min(data1[:, 0]), np.min(data1[:, 1]), np.max(data1[:, 0]), np.max(data1[:, 1]),
        cx_1, cy_1 = (x1 + x2) / 2, (y1 + y2) / 2

    elif isinstance(reg1, Mask):
        assert False, 'Type Error! Mask unsupported!'

    if isinstance(reg2, Rectangle):
        data2 = np.round(reg2._data)
        x1, y1, w, h = data2[:, 0]
        x2, y2 = x1 + w, y1 + h
        cx_2, cy_2 = data2[0, 0] + data2[2, 0] / 2, data2[1, 0] + data2[3, 0] / 2

    elif isinstance(reg2, Polygon):
        data2 = np.round(reg2._points)
        x1, y1, x2, y2 = np.min(data2[:, 0]), np.min(data2[:, 1]), np.max(data2[:, 0]), np.max(data2[:, 1]),
        cx_2, cy_2 = (x1 + x2) / 2, (y1 + y2) / 2

    elif isinstance(reg2, Mask):
        assert False, 'Type Error! Mask unsupported!'
    R2 = (np.power(x2 - x1, 2) + np.power(y2 - y1, 2)) / 4
    P2 = np.power(cx_1 - cx_2, 2) + np.power(cy_1 - cy_2, 2)

    return R2 / (P2 + R2)

def calculate_location_errs(first: List[Region], second: List[Region], bounds: Optional[Tuple[int, int]]):
    """
    first and second are lists containing objects of type Region
    bounds is in the format [width, height]
    output: list of per-frame overlaps (floats)
    ...
    if not len(first) == len(second):
        raise RegionException("List not of the same size {} != {}".format(len(first), len(second)))
    return [calculate_location_err(pairs[0], pairs[1], bounds=bounds) for i, pairs in enumerate(zip(first, second))]
